...
$(document).ready(function() {
	var urlstring = baseUrl + '/detail/listByName';
	/*
	 * limit :为每页的记录数(common.js参得)
	 */
	$('#codeDetailList').datagrid({
		title : 'CodeMasterDetail列表',
		width : '100%',
		nowrap : false,
		striped : true,
		pageNumber : 1,
		...
		onHeaderContextMenu : function(e, field) {
			e.preventDefault();
			if (!$('#tmenu').length) {
				createColumnMenu('#codeDetailList');
			}
			$('#tmenu').menu('show', {
				left : e.pageX,
				top : e.pageY
	...
	var pager = $('#codeDetailList').datagrid('getPager');
	$(pager).pagination({
		showPageList : false
	});
 ...
 * 打开新增页，返回重新加载Grid
 */
function codedetailadd() {
	// alert("add");
	createWindow("jsjladdpage", "codemaster_detail新增", true, "normal", false,
			500, 350, "../pages/mng/codemaster_detail_add.jsp");
}

/*
 * 打开修改页，返回刷新当前页(保留在当前页码以及查询条件不变)
 */
function codedetailedit() {
	// alert("update");
	var id = getSelections();
	if (id) {
		// alert("id="+id);
		var url = "../pages/mng/codemaster_detail_edit.jsp?id=" + id;
		createWindow("jsjleditpage", "codemaster_detail编辑", true, "normal",
				false, 500, 350, url);
	}
}

// 删除选中项
function deleteCodeDetails() {
	var ids = getDeleteSelections();
	if (ids) {
		$.messager.confirm('提示信息', '您是否确认删除所选项?', function(r) {
			if (r) {
				$.ajax({
					url : baseUrl + "/detail/delete?ids=" + ids,
					dataType : "json",
					type : "POST",
					success : function(data) {
						if (data) {
							if (data.success) {
								...
								$('#codeDetailList').datagrid('load');
								clearSelect("codeDetailList");
							} else {
...
function getDeleteSelections() {
	var ids = new Array();
	var rows = $('#codeDetailList').datagrid('getSelections');
