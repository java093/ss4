...
$(document).ready(function(){
	...
	$('#taskList').datagrid({
		...
		onDblClickRow: function(rowIndex, rowData){
			var url = "../pages/job/task_view.jsp?id="+rowData.id;
...
*/ 
function taskadd() {
		createWindow("editTask","子任务新增",true,"normal",false,630,480,"../pages/job/task_add.jsp?id=0");	
}
...
function taskupdate(){
	...
	if(id){
		var url = "../pages/job/task_add.jsp?id="+id;
		createWindow("editTask","子任务编辑",true,"normal",false,630,480,url);	
	}
}
...
function taskview(){
	var id=getSelections();
	if(id){
		var url = "../pages/job/task_view.jsp?id="+id;
		createWindow("viewTask","任务查看",true,"normal",false,630,480,url);	
	}
}
...
function deleteTasks(){
       ...
       $.messager.confirm('提示信息','您是否确认删除所选项?',function(r){ 
    	   if(r){
	        $.ajax({
	  	          url : baseUrl+"/task/delete?ids="+ids+"&type=1",
	  	          dataType : "json",
	  	          type : "POST",	  	  
